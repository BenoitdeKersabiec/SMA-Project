...
class Preferences:
    ...
    def is_item_among_top_10_percent(self, item, item_list):
        ...
        while len(top_items) < n:
            x = self.most_preferred(item_list)
            item_list.remove(x)
            top_items.append(x)
        is_top_item = item in top_items
        return is_top_item
...
if __name__ == '__main__':
    ...
    print('Diesel Engine among top 10 items : {}'.format(agent_pref.is_item_among_top_10_percent(diesel_engine, [diesel_engine, electric_engine])))
